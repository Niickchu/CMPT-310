...
def enhancedFeatureExtractorDigit(datum):
    ...
    -> Edge detection in both directions (x and y)
    -> this technically also is a contigious region detector multiplied by pixel height or width
    ...
    for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            ...
            if datum.getPixel(x, y) != datum.getPixel(x - 1, y):
                features[("horizontal_edge", x, y)] = 1
            else:
                features[("horizontal_edge", x, y)] = 0

            # Vertical edges
            if datum.getPixel(x, y) != datum.getPixel(x, y - 1):
                features[("vertical_edge", x, y)] = 1
            else:
                features[("vertical_edge", x, y)] = 0
...
def runClassifier(args, options):
    ...
    if((options.odds) & (options.classifier == "naiveBayes" or (options.classifier == "nb")) ):
        ...
        print("Computing the odds ratio for label %d and label %d" % (label1, label2))
