...
class BoardGui(Frame):
    ...
    def __init__(self, parent, board):
        ...
        self.nextButton = Button(master=parent, text="next", borderwidth=1, border=1, command=self.next)
        self.nextButton.grid(row=self.dims, column=4, padx=1, pady=5)


        depthLabel = Label(master=parent, text="Depth:", width=10)
        depthLabel.grid(row=self.dims, column=5, padx=1, pady=5)
        self.depthStr.set(str(self.depth))
        self.depthStr.trace("w", self.entry_changed)
        depthEntry = Entry(parent, width=7, textvariable=self.depthStr)
        ...
        self.player1Label = Label(master=parent, text="LivePieces_1:" + str(self.player1.livePieces), fg="white",
                                  bg="blue", width=15)
        self.player1Label.grid(row=self.dims + 1, column=0, padx=1, pady=2)
        self.player2Label = Label(master=parent, text="LivePieces_2:" + str(self.player2.livePieces), fg="white",
                                  bg="red", width=15)
    ...
    def entry_changed(self, *args):
        # This function will be called whenever the Entry's content changes.
        depth_value = self.depthStr.get()
        if depth_value.isdigit():
            self.depth = int(depth_value)
            print("Depth changed:", depth_value)
        else:
            print("Depth value is not a number")

    def next(self):  # runs one step of game, both players play one move
        # check if player1 is AI player

        if (self.player1.type == PlayerType[0]):
            print("Player1 is not AI player, cannot play next")
            return
    ...
    def disable_game(self):
        ...
        for row in self.cells:
            for x in row:
                x.button.config(state='disabled')
    ...
    def on_click(self, button):
        ...
        #if player1 is human, do this
        if self.player1.type == PlayerType[0]:
            valid_move = self.human_move(button)
            if valid_move:
                time.sleep(0.25)
                # move opponent
                self.ai_move("O")
    ...
    PlayerType = ["Human", "Random", "MinMax", "AlphaBeta", "AlphaBetaCutoff", "ExpectimaxCutoff"]
    def ai_move(self, player):  #player is a string
        curPlayerType = self.player1.type
        if player == "O":
            curPlayerType = self.player2.type

        if curPlayerType == PlayerType[1]: #Random
            self.randomPlayerMove(player)

        elif curPlayerType == PlayerType[2]: #Minimax
            self.minimaxPlayerMove(player)

        elif curPlayerType == PlayerType[3]: #AlphaBeta
            self.alphaBetaPlayerMove(player)

        elif curPlayerType == PlayerType[4]: #AlphaBetaCutoff
            self.alphaBetaCutoffPlayerMove(player)

        elif curPlayerType == PlayerType[5]: #ExpectimaxCutoff
            self.expectimaxCutoffPlayerMove(player)
    def human_move(self, button):
        ...
        #called twice to make move
        else:  # means we are in phase 2 mode, meaning player need to move a piece.
            # Moving logic below has 2 steps: to move a piece, player first select a piece by 'pick'ing a piece in first step. Then in next click, it will pick where
            ...
            if (x, y) in self.player1.poses:
                ...
                valid_move = self.move(start, (x, y), self.player1.sym)
                if valid_move:
    ...
    def minimaxPlayerMove(self, player):
        ...
        if curPlayer.step == GameSteps[0]:
            ...
            if len(curPlayer.poses) < curPlayer.livePieces:  # means we are in phase 1 of the game still putting down new pieces
                ...
                print("minimaxPlayerMove: button.text=", button['text'], "pos: ", x, ", ", y)
            ...
            second_phase( gen_state(to_move=player, x_positions=self.player1.poses, o_positions=self.player2.poses, h=6, v=6),
                self.game, 'minmax', potentialMoves)
    ...
    def alphaBetaPlayerMove(self, player):
        curPlayer = self.player1
        if player == "O":
            curPlayer = self.player2

        if curPlayer.step == GameSteps[0]:
            x, y = alpha_beta_search(gen_state(to_move=player, x_positions=self.player1.poses, o_positions=self.player2.poses, h=6, v=6), self.game)
            ...
            if len(curPlayer.poses) < curPlayer.livePieces:  # means we are in phase 1 of the game still putting down new pieces
                ...
                button.config(text=player, state='disabled', disabledforeground="green")
                print("alphaBetaPlayerMove: button.text=", button['text'], "pos: ", x, ", ", y)
                self.checkMillForPlayer(curPlayer, (x, y))
                if len(curPlayer.poses) == curPlayer.livePieces:
                    curPlayer.step = GameSteps[1]
                    self.enablePlayerCells(curPlayer.poses)
            ...
            potentialMoves = self.findPossibleMoves(curPlayer)  # find all possible moves for this player
            second_phase(gen_state(to_move=player, x_positions=self.player1.poses, o_positions=self.player2.poses, h=6, v=6),
                self.game, 'alpha_beta', potentialMoves)
            if len(potentialMoves) == 0:
                print("!No more move possible for player ", player)
                return
    ...
    def alphaBetaCutoffPlayerMove(self, player):
        curPlayer = self.player1
        if player == "O":
            curPlayer = self.player2

        if curPlayer.step == GameSteps[0]:
            x, y = alpha_beta_cutoff_search(gen_state(to_move=player, x_positions=self.player1.poses, o_positions=self.player2.poses, h=6, v=6), self.game, self.depth) #, eval_fn=self.eval_fn)
            if len(curPlayer.poses) < curPlayer.livePieces:  # means we are in phase 1 of the game still putting down new pieces
                curPlayer.poses.append((x, y))
                button = self.getButton((x, y))
                if button is None:
                ...
                button.config(text=player, state='disabled', disabledforeground="green")
                print("alphaBetaCutoffPlayerMove: button.text=", button['text'], "pos: ", x, ", ", y)
                self.checkMillForPlayer(curPlayer, (x, y))
                if len(curPlayer.poses) == curPlayer.livePieces:
                    curPlayer.step = GameSteps[1]
                    self.enablePlayerCells(curPlayer.poses)
            ...
            potentialMoves = self.findPossibleMoves(curPlayer)  # find all possible moves for this player
            second_phase(gen_state(to_move=player, x_positions=self.player1.poses, o_positions=self.player2.poses, h=6, v=6), self.game,
                         'alpha_beta_cutoff', potentialMoves)
            if len(potentialMoves) == 0:
                print("!No more move possible for player ", player)
                return
    ...
    def expectimaxCutoffPlayerMove(self, player):
        curPlayer = self.player1
        if player == "O":
            curPlayer = self.player2

        if curPlayer.step == GameSteps[0]:
            x, y = expect_minmax(gen_state(to_move=player, x_positions=self.player1.poses, o_positions=self.player2.poses, h=6, v=6), self.game)
            if len(curPlayer.poses) < curPlayer.livePieces:  # means we are in phase 1 of the game still putting down new pieces
                curPlayer.poses.append((x, y))
                button = self.getButton((x, y))
                if button is None:
                ...
                for eCell in self.cells[a]:
                    if eCell.pos == list(end):
                        if eCell.button["text"] != "":
                            ...
                            print("move: The move from ", str(start), " to ", str(end),
                                  " is not legal. Ignore the move request.")
                            return False
