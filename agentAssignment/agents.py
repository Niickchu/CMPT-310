...
class Thing:
    ...
    def is_alive(self):
        ...
        return hasattr(self, 'alive') and self.alive  # if is capable of having alive attribute and it is alive
...
class Environment:
    ...
    def run(self, steps=1000):
        ...
        for step in range(steps):
            if self.is_done():  # if no agent is alive
                return
            self.step()
...
class XYEnvironment(Environment):
    ...
    def move_to(self, thing, destination):
        ...
        if thing.location[1] < self.height / 2:  # check if thing.loc is in lower half of the grid
            if destination[1] >= self.height / 2:  # check if destination is in upper half of the grid
                thing.bump = True
                return thing.bump

        if thing.location[1] >= self.height / 2: # check if thing.loc is in upper half of the grid
            if destination[1] < self.height / 2: # check if destination is in lower half of the grid
                thing.bump = True
                return thing.bump

        obstacle = self.some_things_at(destination, Obstacle)
        other_agent = self.some_things_at(destination, Agent)

        thing.bump = obstacle or other_agent
    ...
    def random_location_inbounds(self, exclude=None):
        ...
        location = (random.randint(self.x_start + 1, self.x_end - 1), random.randint(self.y_start + 1, self.y_end - 1))
        if exclude is not None:
            while location == exclude:
                location = (
                random.randint(self.x_start + 1, self.x_end - 1), random.randint(self.y_start + 1, self.y_end - 1))
        return location

    def random_location_inbounds_upper(self):
        """Returns a random location that is inbounds (within walls if we have walls)"""
        return (random.randint(self.x_start, self.x_end - 1), random.randint(self.height / 2, self.y_end - 1))

    def random_location_inbounds_lower(self):
        """Returns a random location that is inbounds (within walls if we have walls)"""
        return (random.randint(self.x_start, self.x_end - 1), random.randint(self.y_start, self.height / 2 - 1))
...
class VacuumEnvironment(XYEnvironment):
    ...
    def percept(self, agent):
        ...
        return status, bump, agent.location, agent.direction
